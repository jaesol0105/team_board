...
class MembersActivity : BaseActivity() {
    ...
    private fun setupActionBar() {
        ...
        if (actionBar != null) {
            ...
            actionBar.title = getString(R.string.members_menu_id)
        }
    ...
    fun memberDetails(user: User) {
        mBoardDetails.assignedTo.add(user.id) // 사용자 ID를 보드의 멤버 목록에 추가한다.
    ...
    private inner class SendNotificationToUserAsyncTask(val boardName: String, val token: String) : AsyncTask<Any, Void, String>() {
        ...
        // 백그라운드에서 실행될 작업 : 네트워크 작업
        override fun doInBackground(vararg params: Any): String {
            var result: String

            var connection: HttpURLConnection? = null // POST 요청을 수행하기 위해 HTTP 연결 설정
            ...
            try {
                ...
                val serviceAccountStream = resources.assets.open("teamboard-1451c-firebase-adminsdk-hf699-2bf31dbf12.json")
                ...
                /** URL 연결은 입력 및 출력에 사용될 수 있다.
                출력에 사용하고자 하는 경우 DoOutput 플래그를 true로 설정 (기본 값은 false) */
                connection.doOutput = true // 데이터를 받기 위한 설정
                connection.doInput = true // 데이터를 보내기 위한 설정

                connection.instanceFollowRedirects = false // HTTP 요청에서 리디렉션이 발생할 때, 리디렉션을 자동으로 따를지 여부
                connection.requestMethod = "POST" // URL의 요청 메서드를 설정 : POST

                connection.setRequestProperty("Content-Type", "application/json; charset=utf-8")
                connection.setRequestProperty("charset", "utf-8")
                connection.setRequestProperty("Accept", "application/json")

                connection.setRequestProperty("Authorization", "Bearer $accessToken") // 토큰을 통해 요청의 인증을 처리함
                connection.useCaches = false  // 캐시를 무시함
                ...
                val jsonRequest = JSONObject().apply {
                    put("message", JSONObject().apply {
                        ...
                        put("data", JSONObject().apply {
                            ...
                            put("body", "${mAssignedMembersList[0].name}님이 당신을 ${boardName}에 초대했습니다.")
                        })
                    })
                }
                ...
                wr.write(jsonBytes) // JSON 데이터를 DataOutputStream을 사용하여 FCM 서버에 전송한다
                wr.flush()
                wr.close()
                ...
                if (httpResult == HttpURLConnection.HTTP_OK) { // HTTP_OK(200)
                    ...
                    result = connection.responseMessage // HTTP 응답 메시지를(있는 경우) status code와 함께 받음
            ...
            return result // 결과를 PostExecute에 통지
        }
